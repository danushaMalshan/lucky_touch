...
class SilverTicketsScreen extends StatefulWidget {
  const SilverTicketsScreen({Key? key}) : super(key: key);

  @override
  State<SilverTicketsScreen> createState() => _SilverTicketsScreenState();
}

class _SilverTicketsScreenState extends State<SilverTicketsScreen> {
  ...
  _createPayment(BuildContext context, ticketId, amount, ticketType) async {
    ...
    String apiKey =
        context.read<ModelSilverTicketScreen>().apiModel?.apiKey ?? '';
    print('apikey $apiKey');
    Servicewrapper wrapper = new Servicewrapper();
    print('step 1');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Padding(
            ...
            child: FutureBuilder(
              future: context.read<ModelSilverTicketScreen>().getTickets(),
              builder: (context, snapshot) {
                if (snapshot.data != null &&
                    context
                            .watch<ModelSilverTicketScreen>()
                            .model
                            ?.data
                            ?.length !=
                        0) {
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: InkWell(
                onTap: () {
                  context.read<ModelSilverTicketScreen>().getTickets();
