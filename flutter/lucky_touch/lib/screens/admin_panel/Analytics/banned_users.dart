...
class BannedUsersScreen extends StatefulWidget {
  const BannedUsersScreen({Key? key}) : super(key: key);
    ...
    return Container(
      ...
      child: Scaffold(
            ...
            flexibleSpace: Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    '${context.watch<ModelBanUsers>().model?.data?.length ?? ''}',
                    style: TextStyle(
                        color: Colors.white,
                        fontSize: 35,
        ...
        body: Padding(
            ...
            child: FutureBuilder(
              future: context.read<ModelBanUsers>().getBanUsers(),
              builder: (context, snapshot) {
                if (snapshot.data != null &&
                    context.watch<ModelBanUsers>().model!.data!.length > 0) {
                  return ListView.builder(
                      itemCount:
                          context.watch<ModelBanUsers>().model?.data?.length,
                      itemBuilder: (context, index) {
                        var item =
                            context.watch<ModelBanUsers>().model?.data?[index];
                        return Container(
                          margin:
                ...
                } else if (context.watch<ModelBanUsers>().model?.data?.length ==
                    0) {
                  return Center(
                    child: Text(
                      'No banned users',
