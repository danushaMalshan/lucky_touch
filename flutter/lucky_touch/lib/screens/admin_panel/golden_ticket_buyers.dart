...
class GoldenTicketBuyers extends StatefulWidget {
  const GoldenTicketBuyers({Key? key}) : super(key: key);

  @override
  State<GoldenTicketBuyers> createState() => _GoldenTicketBuyersState();
}

class _GoldenTicketBuyersState extends State<GoldenTicketBuyers> {
  bool progress = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: FutureBuilder(
          future: context.read<ModelGetGoldenTicketBuyers>().getTicketBuyers(),
          builder: (context, snapshot) {
            if (snapshot.data != null &&
                context
                        .watch<ModelGetGoldenTicketBuyers>()
                        .model
                        ?.data
                        ?.length !=
                    0) {
              return ListView.builder(
                  ...
                  itemBuilder: ((context, index) {
                    ...
                    var item =
                        context.watch<ModelGetGoldenTicketBuyers>().model?.data;
                                    ...
                                    onChanged: (String? newValue) {
                                      setState(() {
                                        ...
                                        _updateRank(item?[index]?.buyerId ?? 0,
                                            newValue);
                                        context
                                            .read<ModelGetGoldenTicketBuyers>()
                                            .getTicketBuyers();
                                      });
                                    },
                                  )
